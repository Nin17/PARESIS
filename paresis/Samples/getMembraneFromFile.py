...
import os
import time
from InputOutput.pagaille_io import save_edf, open_image
...
def getMembraneFromFile(myMembraneFile,studyDimensions,studyPixelSize, oversamp, numPoint):
    ...
    fullImagePaths.sort()
    thickness=(open_image(fullImagePaths[numPoint]))
    if not all(i==j for i, j in zip(studyDimensions, thickness.shape)):
        raise ValueError("The membrane you are trying to load does not have the correct dimensions")
        # TODO this is wrong! Should resize the image instead
...
if __name__ == "__main__":
    ...
    for pointNum in range (number_of_positions):
        ...
        save_edf(membraneThickness, storingFolder+txtPoint+'.edf')
